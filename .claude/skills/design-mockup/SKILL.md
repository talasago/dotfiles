...
- 今回変更する予定の箇所だけでなく、**実装対象のコンポーネント・画面全体**をデザインの該当セクションと突き合わせる。文言・要素の並び順・スタイルについて、変更範囲外の既存コードも含めて一致しているか確認する。
- 変更箇所だけを見て「差分がない」と判断しない。対象コンポーネント全体をデザインの該当ブロックと照合してから実装・レビューする。
